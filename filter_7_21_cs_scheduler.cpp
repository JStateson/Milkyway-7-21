...
// filter messages:  if filter file exists then enable "low" filter
// if filter file has content then apply filter.
// todo:  issue message once for each project not just once altogether
static bool bOnce = true; // jys
struct EXCLUDE_PROJ_MSG { //jys
    std::string content;  // phrase has this content it is excluded
    bool bMentioned;		// want to mention the message once
};
static std::vector<EXCLUDE_PROJ_MSG> exclude_proj_msgs;
static int NumExcluded = 0;
static bool bLowExcluded = false;
void GetUnwantedList()
{
    FILE* f = fopen("MsgExcludeList.cfg", "r");
    NumExcluded = 0;
    if (f) {
        char buf[256];
        bLowExcluded = false;
    ...
    if (NumExcluded > 0)
    {
        msg_printf(0, MSG_INFO, "Skipping LOW priority and applying %d excluded messages against all projects", NumExcluded);
    }
    else if(bLowExcluded) msg_printf(0, MSG_INFO, "Only skipping LOW priority messages");
    msg_printf(0, MSG_INFO, "Task aborts, suspends and resume are ignored in the version");

}

// see if unwanted content is included
bool bIsExcluded(USER_MESSAGE& um)
{
    int i = -1;
    if (bLowExcluded)
    {
        if (strcmp(um.priority.c_str(), "low") == 0)return true;
    }
    if (NumExcluded == 0)return false; // if no table then do not filter messages
    // if table exists and priority is low then do not print
    for (auto epm : exclude_proj_msgs) {
        i++;
        if (um.message.find(epm.content) != std::string::npos) // found phrase
        {
            if (epm.bMentioned)return true;
            exclude_proj_msgs[i].bMentioned = true;
            return false; // let it print once.
        }
    }
    return false;
}
...
// 
    ...
    if (bInitRatio)
    {
        ...
        bInitRatio = false;
        if (true)
            msg_printf(0, MSG_INFO, "Milkyway fix: Using %d for low water and %d for high water pcts",
